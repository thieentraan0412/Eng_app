...
contextBridge.exposeInMainWorld('api', {
  ...
  // (Cửa sổ chính) Nghe yêu cầu "Lưu vào bộ từ" đến từ popup toàn cục
  onQuickSave: (cb: (entry: SaveEntry) => void): (() => void) => {
    const h = (_e: IpcRendererEvent, entry: SaveEntry) => cb(entry)
  ...
  // (Cửa sổ popup) Lưu từ / đóng popup
  saveDesktopTranslate: (entry: SaveEntry): Promise<boolean> =>
    ipcRenderer.invoke('desktop-translate:save', entry),
